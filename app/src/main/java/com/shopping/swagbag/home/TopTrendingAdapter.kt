...
class TopTrendingAdapter(
    ...
    private val data: List<ParticularCategoryModel.Result.Section>
) :
    ...
    inner class MyViewHolder(private val viewBinding: SingleTopTrendingBinding) :
        ...
        fun bind(singleData: ParticularCategoryModel.Result.Section){
            with(viewBinding){
                // set image
                Glide.with(context).load(singleData.file).into(viewBinding.productImg)
            }
        }
